...
| **测试集** | `/data/test_data/model_ready_panel_selected_plus_ohlc_test_{YEAR}.parquet` | 🔒 只读 | `YEAR∈[2024,2026]`，结构同训练集，用于样本外验证 |
| **原始行情** | `/data/data_process/4.15_revision/raw_daily_panel.parquet` | 🔒 只读 | 用于计算真实日收益率 f`label_{REBALANCE_DAYS}]` (`S_DQ_ADJCLOSE.pct_change().shift(-REBALANCE_DAYS)`) |
| **加工数据** | `/data/cye_temp/processed_data/backtest/` | ✏️ 可读写 | 缓存对齐后的面板数据、特征掩码、回测中间状态 |

> ⚠️ **数据对齐原则**：所有因子与标签均基于 `TRADE_DT` 与 `S_INFO_WINDCODE` 联合主键进行 `left join`。缺失的 `label_i` 默认填充 nan，实际训练中通过 `dropna` 过滤无效样本。
...
```mermaid
graph LR
A[加载年度Parquet] --> B[计算T+i日收益率label_i]
B --> C[特征过滤与掩码应用]
C --> D{回测主循环 run()}
D --> E[模型训练: 仅记录现金净值]
...
- **`BacktestEngine` 类**：回测调度中枢。
  - `run()` 主循环按日遍历：
    1. **收益历史更新**：记录 `label_i`（保留最近 80 天，OptSharpe 截取后 30 天）。
...
| `SLIPPAGE` / `STAMP_TAX_RATE` | 0.000 / 0.0000 | 暂时不考虑真实交易摩擦。回测负收益常因未计入滑点与印花税导致虚高实盘亏损。 |
...
1. **严格防前视偏差**：
   - 训练集时间窗口硬编码截止至 `T-i`，确保 `label_i`（T→T+i）在预测时未泄漏。
...
| `backtest_engine.py` | 使用 `FWD_RET_5D` 训练 | 切换至 `label_i` 训练，匹配日频调仓周期；或改为滚动截面标准化 |
