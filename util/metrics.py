...
#     return sharpe
# def evaluate_and_plot(results: Dict[str, pd.DataFrame], 
#                       output_dir: str, 
#                       figure_dir: str, 
#                       ic_ir_dict: Optional[Dict] = None, 
#                       start_date='', 
#                       TOP_K=50, 
#                       reba_wd='', 
#                       suffix=''):
...
#     plt.legend()
#     plt.tight_layout()
#     plt.savefig(os.path.join(figure_dir, f'pnl_combined_top{TOP_K}_{start_date}_{reba_wd}{suffix}.png'), dpi=150)
#     plt.close()
...
#     plt.legend()
#     plt.tight_layout()
#     plt.savefig(os.path.join(figure_dir, f'drawdown_combined_top{TOP_K}_{start_date}_{reba_wd}{suffix}.png'), dpi=150)
#     plt.close()
...
#     plt.legend()
#     plt.tight_layout()
#     plt.savefig(os.path.join(figure_dir, f'rolling_sharpe_combined_top{TOP_K}_{start_date}_{reba_wd}{suffix}.png'), dpi=150)
#     plt.close()
...
#     # ─────────────────────────────────────────────────────────────
#     if ic_ir_dict:
#         ic_path = os.path.join(output_dir, f'factor_ic_ir_top{TOP_K}_{start_date}_{reba_wd}.json')
...
def evaluate_and_plot(results: Dict[str, pd.DataFrame],
    ...
    if df is None or df.empty:
        logging.warning("⚠️ 无有效回测数据，跳过绘图")
        return {}
        
    df = df.dropna(subset=['TRADE_DT']).sort_values('TRADE_DT').reset_index(drop=True)
    df['TRADE_DT'] = pd.to_datetime(df['TRADE_DT'])
    
    # 🔑 强化样本外评估：强制只绘制和计算指定 start_date 之后的绩效
    test_start_date = pd.to_datetime(start_date) if start_date else pd.to_datetime('2025-01-01')
    df_test = df[df['TRADE_DT'] >= test_start_date].copy()
    
    if df_test.empty:
        logging.warning("⚠️ 测试集时间段内无有效数据，跳过绘图")
        return {}
    ...
    # ─────────────────────────────────────────────────────────────
    # 步骤 1: 计算所有模型的绩效指标 (用于排序和 JSON 导出)
    # ─────────────────────────────────────────────────────────────
    for name in results.keys():
        col = f'Value_{name}'
        if col not in df_test.columns or df_test[col].isna().all():
    ...
    # 🔑 步骤 2: 按最终总收益 (total_return) 降序排序，筛选出前 10 名用于绘图
    sorted_models = sorted(
        metrics_summary.keys(),
        key=lambda x: metrics_summary[x].get('total_return', -np.inf),
        reverse=True
    )
    top_10_models = sorted_models[:10]
    logging.info(f"🏆 绘图筛选：最终 PnL 前 10 的模型为: {top_10_models}")

    # ─────────────────────────────────────────────────────────────
    # 图 1: 累计净值曲线 (仅绘制 Top 10)
    # ─────────────────────────────────────────────────────────────
    plt.figure(figsize=(14, 6))
    for name in top_10_models:
        col = f'Value_{name}'
    ...
    plt.title(f'Out-of-Sample Cumulative PnL (Top 10 by Total Return) | Top K={TOP_K}')
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
    plt.grid(True, alpha=0.3)
    ...
    plt.title('Strategy Drawdown Comparison (Top 10 by Total Return)')
    plt.grid(True, alpha=0.3)
    ...
    # ─────────────────────────────────────────────────────────────
    # 图 3: 30 日滚动夏普比率 (仅绘制 Top 10)
    # ─────────────────────────────────────────────────────────────
    plt.figure(figsize=(14, 4))
    for name in top_10_models:
        col = f'Value_{name}'
        if col not in df_test.columns or len(df_test[col]) < 30:
            continue
        r_sharpe = compute_rolling_sharpe(df_test[col], window=30)
        valid_mask = r_sharpe.notna()
        if valid_mask.any():
    ...
    plt.axhline(0, color='k', linestyle='--', lw=0.8, alpha=0.5)
    plt.title('30-Day Rolling Sharpe Ratio (Top 10 by Total Return)')
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))
    plt.grid(True, alpha=0.3)
    plt.legend(loc='upper left', fontsize='small')
    plt.tight_layout()
    ...
    # ─────────────────────────────────────────────────────────────
    if ic_ir_dict:
        ic_path = os.path.join(output_dir, f'factor_ic_ir_top{TOP_K}_{start_date}_{reba_wd}{suffix}.json')
        with open(ic_path, 'w', encoding='utf-8') as f:
            json.dump(ic_ir_dict, f, indent=2, ensure_ascii=False)
        logging.info(f"✅ IC/IR 报告已保存至: {ic_path}")
